export default class SudokuGenerator {
    ...
    shuffleGrid() {
        let nums, chars, shuffledNums, shuffledChars, index1, index2, index3, index, rotate, numMap, charMap, tempGrid, grid
        grid = this.grid
    ...
    generate() {
        ...
        return [
            puzzle.map(item => item.map(i => Number(i))),
            solution.map(item => item.map(i => Number(i)))
        ]
    }
}
